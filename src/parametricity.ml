...
let rec relation order evd env (t : constr) : constr =
 ...
 if !debug_mode && List.exists (fun x -> List.mem x [`Relation]) debug_flag then begin
    ...
    let lams = range (fun k -> RelDecl.LocalAssum (Context.make_annot (prime_name order k na) rel, lift k (prime env.env evd order k t))) order in
...
and translate order evd env (t : constr) : constr =
  ...
  let res = match kind !evd t with
    ...
    | Sort _ | Prod (_,_,_) ->
        ...
        let na = Namegen.named_hd env.env !evd t Anonymous in
        let rel = Retyping.relevance_of_type env.env !evd t in
        let lams = range (fun k -> (Context.make_annot (prime_name order k na) rel, lift k (prime env.env evd order k t))) order in
    ...
    | App (c,l) ->
        ...
        let l_R = List.flatten (List.map (fun x ->
           ...
           (range (fun k -> prime env.env evd order k x) order)) l) in
    ...
    | Cast (c, k, t) ->
        ...
        let sub = range (fun k -> prime env.env evd order k c) order in
    ...
    | Case (ci, u, pms, p, iv, c, bl) ->
        ...
        let sub = range (fun k -> prime env.env evd order k theta) order in
...
| RelDecl.LocalAssum (na, t) ->
  let lams = range (fun k -> RelDecl.LocalAssum (Context.map_annot (prime_name order k) na, lift k (prime env.env evd order k t))) order in
  let na_R = Context.map_annot (translate_name order) na in
  let t_R = relation order evd env t in
  RelDecl.LocalAssum (na_R, t_R) :: lams
| RelDecl.LocalDef (na, body, t) ->
  let lets = range (fun k -> RelDecl.LocalDef (Context.map_annot (prime_name order k) na, lift k (prime env.env evd order k body), lift k (prime env.env evd order k t))) order in
  let na_R = Context.map_annot (translate_name order) na in
  let t_R = relation order evd env t in
...
and translate_cofix order evd env t =
  ...
  let nfun = Array.length lna in
  let tl_k =
    let map t = Array.init order (fun k -> prime env.env evd order k t) in
    Array.map map tl
  in
  let rec letfix name fix n k acc =
    if k = 0 then acc
      ...
      let fix_k = lift (n*order + k) (prime env.env evd order k fix) in
  ...
  let tl_R = Array.mapi (fun n (ft, ft_R, bk, bk_R) ->
     ...
     let sub = range (fun k ->
                  mkApp (mkRel (ft_R_len + (nfun - n)*order - k ),
                     Array.map (prime env.env evd order k) (Context.Rel.instance mkRel 0 ft)))
               order
     in
...
and rewrite_fixpoints order evdr env (depth : int) (fix : fixpoint) source target typ typ_R acc =
  ...
  let env_R =
    if List.exists (fun x -> List.mem x [`Fix]) debug_flag then begin
      ...
      let rc_order = rev_range (fun k -> RelDecl.LocalAssum (Context.make_annot (Name (Id.of_string (Printf.sprintf "rel_%d" k))) rel,
                                         lift k (prime env.env evdr order k typ))) order in
  ...
  let res1 =
    (fold_nat (fun k acc ->
    let pred_sub =
      (range (fun x -> lift 1 (prime env.env evdr order (k+1+x) target)) (order-1 - k))
      @ [ mkRel 1 ]
      @ (range (fun x -> lift 1 (prime env.env evdr order x source)) k)
    in
    ...
    let index = lift 1 (prime env.env evdr order k typ) in
    ...
    let base = lift 1 (prime env.env evdr order k source) in
    let endpoint = lift 1 (prime env.env evdr order k target) in
    let path = mkApp (mkRel 1,
       Array.map (fun x -> lift 1 (prime env.env evdr order k x)) new_vec)
    in
...
and rewrite_cofixpoints order evdr env (depth : int) (fix : cofixpoint) source target typ typ_R acc =
  ...
  let_gen @@ (fold_nat (fun k acc ->
    ...
    let endpoint = lift 1 (prime env.env evdr order k target) in
    let path = mkApp (mkRel 1,
       Array.map (fun x -> lift 1 (prime env.env evdr order k x))
